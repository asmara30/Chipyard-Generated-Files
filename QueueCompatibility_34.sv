...
module QueueCompatibility_34(
  ...
  reg  [7:0] enq_ptr_value;	// @[Counter.scala:61:40]
  reg  [7:0] deq_ptr_value;	// @[Counter.scala:61:40]
  ...
  always @(posedge clock) begin
    if (reset) begin
      enq_ptr_value <= 8'h0;	// @[Counter.scala:61:40]
      deq_ptr_value <= 8'h0;	// @[Counter.scala:61:40]
      maybe_full <= 1'h0;	// @[Decoupled.scala:276:27]
    end
    else begin
      if (do_enq)	// @[Decoupled.scala:51:35]
        enq_ptr_value <= enq_ptr_value + 8'h1;	// @[Counter.scala:61:40, :77:24]
      if (do_deq)	// @[Decoupled.scala:51:35]
        deq_ptr_value <= deq_ptr_value + 8'h1;	// @[Counter.scala:61:40, :77:24]
  ...
  `ifndef SYNTHESIS
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        _RANDOM_0 = `RANDOM;
        enq_ptr_value = _RANDOM_0[7:0];	// @[Counter.scala:61:40]
        deq_ptr_value = _RANDOM_0[15:8];	// @[Counter.scala:61:40]
        maybe_full = _RANDOM_0[16];	// @[Counter.scala:61:40, Decoupled.scala:276:27]
      `endif // RANDOMIZE_REG_INIT
    end // initial
  ...
  assign io_count = {full, enq_ptr_value - deq_ptr_value};	// @[Counter.scala:61:40, Decoupled.scala:279:24, :326:32, :329:62]
endmodule
