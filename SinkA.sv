...
module SinkA(
  ...
  wire [39:0] _freeOH_T_3 = _freeOH_T_22 | {_freeOH_T_22[38:0], 1'h0};	// @[SinkA.scala:58:25, package.scala:245:{43,48,53}]
  wire [39:0] _freeOH_T_6 = _freeOH_T_3 | {_freeOH_T_3[37:0], 2'h0};	// @[Edges.scala:228:27, package.scala:245:{43,53}]
  ...
  wire [40:0] freeOH = {~(_freeOH_T_15 | {_freeOH_T_15[7:0], 32'h0}), 1'h1} & {1'h0, _freeOH_T_22};	// @[Edges.scala:229:28, SinkA.scala:58:{16,25,33,39}, package.scala:245:{43,48,53}]
  wire [30:0] _GEN = {23'h0, freeOH[40:33]} | freeOH[31:1];	// @[OneHot.scala:31:18, :32:28, SinkA.scala:58:39]
  ...
  wire [5:0]  freeIdx = {|(freeOH[40:32]), |(_GEN[30:15]), |(_GEN_0[14:7]), |(_GEN_1[6:3]), |(_GEN_2[2:1]), _GEN_2[2] | _GEN_2[0]};	// @[Cat.scala:33:92, OneHot.scala:30:18, :31:18, :32:{14,28}, SinkA.scala:58:39]
  reg  [1:0]  first_counter;	// @[Edges.scala:228:27]
  wire        first = first_counter == 2'h0;	// @[Edges.scala:228:27, :230:25]
  ...
  wire        set_block = ~(io_a_bits_opcode[2]) & first & (&lists);	// @[Edges.scala:91:{28,37}, :230:25, SinkA.scala:51:22, :57:21, :71:36]
  wire        _io_a_ready_output = ~req_block & ~buf_block & ~set_block;	// @[SinkA.scala:69:25, :70:27, :71:36, :77:{14,28,39,42}]
  wire        _T_3 = io_a_valid & first;	// @[Edges.scala:230:25, SinkA.scala:78:27]
  reg  [5:0]  put_r;	// @[Reg.scala:19:16]
  wire [5:0]  put = first ? freeIdx : put_r;	// @[Cat.scala:33:92, Edges.scala:230:25, Reg.scala:19:16, SinkA.scala:83:16]
  wire        _T_9 = _io_pb_pop_ready_output & io_pb_pop_valid;	// @[Decoupled.scala:51:35, SinkA.scala:104:40]
  ...
  always @(posedge clock) begin
    if (reset) begin
      ...
      first_counter <= 2'h0;	// @[Edges.scala:228:27]
    end
    else begin
      lists <= (lists | (_T_3 & ~(io_a_bits_opcode[2]) & ~req_block & ~buf_block ? freeOH[39:0] : 40'h0)) & ~(_T_9 & io_pb_pop_bits_last ? _lists_clr_T[39:0] : 40'h0);	// @[Decoupled.scala:51:35, Edges.scala:91:{28,37}, OneHot.scala:64:{12,27}, SinkA.scala:51:22, :55:{19,32,34}, :58:39, :69:25, :70:27, :77:{14,28}, :78:27, :80:{51,66,78}, :107:{26,50}, :108:15, compatibility.scala:76:26]
      if (_io_a_ready_output & io_a_valid) begin	// @[Decoupled.scala:51:35, SinkA.scala:77:39]
        if (first) begin	// @[Edges.scala:230:25]
          if (io_a_bits_opcode[2])	// @[Edges.scala:91:37]
            first_counter <= 2'h0;	// @[Edges.scala:228:27]
          else	// @[Edges.scala:91:37]
            first_counter <= ~(_first_beats1_decode_T_1[5:4]);	// @[Edges.scala:228:27, package.scala:235:{46,71,76}]
        end
        else	// @[Edges.scala:230:25]
          first_counter <= first_counter - 2'h1;	// @[Edges.scala:228:27, :229:28]
      end
    end
  ...
  `ifndef SYNTHESIS
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        _RANDOM_0 = `RANDOM;
        _RANDOM_1 = `RANDOM;
        lists = {_RANDOM_0, _RANDOM_1[7:0]};	// @[SinkA.scala:51:22]
        first_counter = _RANDOM_1[9:8];	// @[Edges.scala:228:27, SinkA.scala:51:22]
        put_r = _RANDOM_1[15:10];	// @[Reg.scala:19:16, SinkA.scala:51:22]
      `endif // RANDOMIZE_REG_INIT
    end // initial
  ...
  ListBuffer putbuffer (	// @[SinkA.scala:50:25]
    .clock                     (clock),
    .reset                     (reset),
    .io_push_valid             (io_a_valid & ~(io_a_bits_opcode[2]) & ~req_block & ~set_block),	// @[Edges.scala:91:{28,37}, SinkA.scala:69:25, :71:36, :77:{14,42}, :79:63]
  ...
  assign io_req_valid = _T_3 & ~buf_block & ~set_block;	// @[SinkA.scala:70:27, :71:36, :77:{28,42}, :78:{27,50}]
  ...
  assign io_req_bits_tag = {io_a_bits_address[31], io_a_bits_address[27:16]};	// @[Parameters.scala:212:47, :213:22, :214:19]
  assign io_req_bits_offset = io_a_bits_address[5:0];	// @[Parameters.scala:215:50]
  assign io_req_bits_put = put;	// @[SinkA.scala:83:16]
  assign io_req_bits_set = io_a_bits_address[15:6];	// @[Parameters.scala:213:22, :215:28]
