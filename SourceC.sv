...
module SourceC(
  ...
  reg  [3:0]  req_r_source;	// @[Reg.scala:19:16]
  reg  [12:0] req_r_tag;	// @[Reg.scala:19:16]
  reg  [9:0]  req_r_set;	// @[Reg.scala:19:16]
  reg  [2:0]  req_r_way;	// @[Reg.scala:19:16]
  wire [9:0]  req_set = busy ? req_r_set : io_req_bits_set;	// @[Reg.scala:19:16, SourceC.scala:65:21, :68:17]
  wire [2:0]  req_way = busy ? req_r_way : io_req_bits_way;	// @[Reg.scala:19:16, SourceC.scala:65:21, :68:17]
  wire        _T_14 = io_req_valid & room;	// @[SourceC.scala:58:21, :69:41]
  wire        want_data = busy | _T_14 & io_req_bits_dirty;	// @[SourceC.scala:65:21, :69:{24,41,49}]
  ...
  reg  [3:0]  s2_req_source;	// @[Reg.scala:19:16]
  reg  [12:0] s2_req_tag;	// @[Reg.scala:19:16]
  ...
  reg  [3:0]  s3_req_source;	// @[Reg.scala:19:16]
  reg  [12:0] s3_req_tag;	// @[Reg.scala:19:16]
  reg  [9:0]  s3_req_set;	// @[Reg.scala:19:16]
  wire        _room_T_4 = _queue_io_enq_ready & s3_valid;	// @[Decoupled.scala:51:35, SourceC.scala:53:21, :99:25]
  wire        _s2_latch_T = io_bs_adr_ready & _io_bs_adr_valid_output;	// @[Decoupled.scala:51:35, SourceC.scala:76:50]
  wire        _s2_latch_T_1 = _io_req_ready_output & io_req_valid;	// @[Decoupled.scala:51:35, SourceC.scala:71:25]
  always @(posedge clock) begin
    if (reset) begin
      ...
      if (_room_T_4 != (io_c_ready & _queue_io_deq_valid)) begin	// @[Decoupled.scala:51:35, SourceC.scala:53:21, :59:29]
        fill <= fill + (_room_T_4 ? 4'h1 : 4'hF);	// @[Decoupled.scala:51:35, SourceC.scala:57:21, :60:{18,23,54}]
        room <= fill == 4'h0 | (fill == 4'h1 | fill == 4'h2) & ~_room_T_4;	// @[Decoupled.scala:51:35, SourceC.scala:57:21, :58:21, :60:23, :61:{18,30,40,52,60,73,76}]
      end
      busy <= ~(_s2_latch_T & (&beat)) & (_T_14 & io_req_bits_dirty | busy);	// @[Decoupled.scala:51:35, SourceC.scala:65:21, :66:21, :67:19, :69:41, :86:{30,52,59}, :87:27, :88:{17,24}]
      if (_s2_latch_T)	// @[Decoupled.scala:51:35]
        beat <= beat + 3'h1;	// @[SourceC.scala:60:23, :66:21, :89:18]
    end
    ...
    if (want_data)	// @[SourceC.scala:69:24]
      s2_valid <= _s2_latch_T;	// @[Decoupled.scala:51:35, SourceC.scala:93:25]
    else	// @[SourceC.scala:69:24]
      s2_valid <= _s2_latch_T_1;	// @[Decoupled.scala:51:35, SourceC.scala:93:25]
    if (want_data ? _s2_latch_T : _s2_latch_T_1) begin	// @[Decoupled.scala:51:35, SourceC.scala:69:24, :92:21]
  ...
  `ifndef SYNTHESIS
    always @(posedge clock) begin	// @[SourceC.scala:63:10]
      if (~reset & room != (_queue_io_count < 4'h2)) begin	// @[SourceC.scala:53:21, :58:21, :61:60, :63:{10,16,35}]
      ...
      if (~reset & ~(~s3_valid | _queue_io_enq_ready)) begin	// @[SourceC.scala:53:21, :99:25, :115:{9,10,19}]
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        _RANDOM_2 = `RANDOM;
        _RANDOM_3 = `RANDOM;
        fill = _RANDOM_0[3:0];	// @[SourceC.scala:57:21]
        room = _RANDOM_0[4];	// @[SourceC.scala:57:21, :58:21]
        busy = _RANDOM_0[5];	// @[SourceC.scala:57:21, :65:21]
        beat = _RANDOM_0[8:6];	// @[SourceC.scala:57:21, :66:21]
        req_r_opcode = _RANDOM_0[11:9];	// @[Reg.scala:19:16, SourceC.scala:57:21]
        req_r_param = _RANDOM_0[14:12];	// @[Reg.scala:19:16, SourceC.scala:57:21]
        req_r_source = _RANDOM_0[18:15];	// @[Reg.scala:19:16, SourceC.scala:57:21]
        req_r_tag = _RANDOM_0[31:19];	// @[Reg.scala:19:16, SourceC.scala:57:21]
        req_r_set = _RANDOM_1[9:0];	// @[Reg.scala:19:16]
        req_r_way = _RANDOM_1[12:10];	// @[Reg.scala:19:16]
        s2_valid = _RANDOM_1[14];	// @[Reg.scala:19:16, SourceC.scala:93:25]
        s2_req_opcode = _RANDOM_1[17:15];	// @[Reg.scala:19:16]
        s2_req_param = _RANDOM_1[20:18];	// @[Reg.scala:19:16]
        s2_req_source = _RANDOM_1[24:21];	// @[Reg.scala:19:16]
        s2_req_tag = {_RANDOM_1[31:25], _RANDOM_2[5:0]};	// @[Reg.scala:19:16]
        s2_req_set = _RANDOM_2[15:6];	// @[Reg.scala:19:16]
        s3_valid = _RANDOM_2[24];	// @[Reg.scala:19:16, SourceC.scala:99:25]
        s3_req_opcode = _RANDOM_2[27:25];	// @[Reg.scala:19:16]
        s3_req_param = _RANDOM_2[30:28];	// @[Reg.scala:19:16]
        s3_req_source = {_RANDOM_2[31], _RANDOM_3[2:0]};	// @[Reg.scala:19:16]
        s3_req_tag = _RANDOM_3[15:3];	// @[Reg.scala:19:16]
        s3_req_set = _RANDOM_3[25:16];	// @[Reg.scala:19:16]
      `endif // RANDOMIZE_REG_INIT
    end // initial
  ...
  QueueCompatibility_32 queue (	// @[SourceC.scala:53:21]
    .clock               (clock),
    .reset               (reset),
    ...
    .io_enq_bits_address ({s3_req_tag[12], 3'h0, s3_req_tag[11:0], s3_req_set, 6'h0}),	// @[Cat.scala:33:92, Parameters.scala:219:15, :227:72, Reg.scala:19:16, SourceC.scala:66:21]
